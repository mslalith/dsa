...
class FindNumberOfWaysToPlacePeopleII : TestCaseProblem<Array<IntArray>, Int>() {
    
    companion object {
        @JvmStatic
        fun main(args: Array<String>) = FindNumberOfWaysToPlacePeopleII().runForConsole()
    }
    ...
    private fun numberOfPairs(points: Array<IntArray>): Int {
        ...
        for (i in 0..<n) {
            val (aliceX, aliceY) = points[i]
            var bestBobY = Int.MIN_VALUE
            for (j in i + 1..<n) {
                val (bobX, bobY) = points[j]

                if (bobY > aliceY) continue
